...
class r3dDataset(Dataset):
    ...
    def _getset(self,idx): 
        ...
        image = np.fromstring(target.loc[idx]['image'].replace(' ', ''), 
                          dtype=np.uint8, sep=',').reshape(self.size, self.size, 3)
        boundary = np.fromstring(target.loc[idx]['boundary'].replace(' ', ''), 
                                dtype=np.uint8, sep=',').reshape(self.size, self.size)
        room = np.fromstring(target.loc[idx]['room'].replace(' ', ''), 
                            dtype=np.uint8, sep=',').reshape(self.size, self.size)
        door = np.fromstring(target.loc[idx]['door'].replace(' ', ''), 
                            dtype=np.uint8, sep=',').reshape(self.size, self.size)
